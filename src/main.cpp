...
// a world position for each object in the scene
//vec3 obj_pos_wor[] = {};
//vec3 obj_pos_wor2[][10][10];
...
int main(const int argc, const char *argv[]){
	...
	bool found = false;
	int i = 0;
	/*while(sizeof(obj_rand) < 10)
	{
		 aux = (int)rand()% 10;
		 printf("numero random generado %i\n", aux);
		 for (int e = 0; e < 10; ++e)
		 {
		 	if (aux == obj_rand[e])
		 	{
		 ...
		 {
		 	obj_rand[i]= aux;
		 	printf("agregado  %i pos %i\n", aux, i);
		 }
		 //printf("num arreglo %i %d \n", obj_rand[i], found);
		 found == false;
		 i++;
	}*/

    if(argc != 2){
        fprintf(stderr, "./prog L\n");
        exit(EXIT_FAILURE);
    }
    L = atoi(argv[1]);
    unsigned long L3 = L*L*L;
 	//vec3 obj_pos_wor[L*L*L];
